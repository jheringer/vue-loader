...
module.exports.pitch = function (remainingRequest) {
  ...
  if (query.type === `style`) {
    ...
    if (cssLoaderIndex > -1) {
      ...
      let request = genRequest([...afterLoaders, stylePostLoaderPath, ...beforeLoaders]);

      const vueLoader = this.loaders[this.loaders.length - 1];

      if (vueLoader && vueLoader.options && vueLoader.options.ignoreStylePostLoader) {
          request = genRequest(loaders);
      }
      // console.log(request)
